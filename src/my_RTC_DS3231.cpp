...
bool RTC_DS3231::setAlarm2(const DateTime &dt, Ds3231Alarm2Mode alarm_mode) {
    uint8_t ctrl = read_register(DS3231_CONTROL);
    if (!(ctrl & 0x04)) {
        DPL("ERROR: I2C Read Control Register");
        return false;
    }
    ...
    if (!(i2c_dev->write(buffer, 4))) {
        DPL("ERROR: I2C Write Buffer");
        return false;
    };
...
bool RTC_DS3231::getAlarm2(DateTime *alarm, DateTime current_time) {

    DPL("Check Alarm2");
#define BM_ALARM_2_ENABLED  0b00000010
    uint8_t c = read_register(DS3231_CONTROL);
    boolean cc = (c & BM_ALARM_2_ENABLED) >> 1;

    if (cc) {
        DPL("Alarm2 is enabled!");
    } else {
        DPL("Alarm2 is disabled!");
    }

    uint8_t buffer[3] = {DS3231_ALARM2, 121, 122};
    if(!(i2c_dev->write_then_read(buffer, 1, buffer, 3,false))) {
        DPL("ERROR WRITE");
    }
...
#define  BM_01_MIN 0b00001111
#define  BM_10_MIN 0b01110000
    uint8_t mm = (m & BM_01_MIN) + 10 * ((m & BM_10_MIN) >> 4);

#define BM_01_HH 0b00001111
#define BM_10_HH 0b00010000
#define BM_20_HH 0b00100000
    uint8_t hh = (h & BM_01_HH) + 10 * ((h & BM_10_HH) >> 4) + 20 * ((h & BM_20_HH) >> 5);

    // !!! Assuming date alarm is active
#define BM_01_DATE 0b00001111
#define BM_10_DATE 0b00110000
    uint8_t dd = (d & BM_01_DATE) + 10 * ((d & BM_10_DATE) >> 4);

    uint16_t yy = current_time.year();
    uint8_t mo = current_time.month();

    // Alarm clock does not know month, we can take current month/year - if alarm is on next month/month end, we need to jump to next month
    if (current_time.day() > dd) {
        mo++;
        if (mo > 12) {
            mo = 1;
            yy++;
    ...
    *alarm = DateTime(yy, mo, dd, hh, mm, 0);

    DPL("***********************");
    DPF("M:%i H:%i D:%i\n", m, h, d);
    DPF("%i:%i  - DD:%i MM:%i YY:%i \n", hh, mm, dd, mm, yy);
    DPL("***********************");
...
bool RTC_DS3231::getAlarm1(DateTime *alarm, DateTime current_time) {
    DPL("Check Alarm1");
#define BM_ALARM_2_ENABLED  0b00000010
#define BM_ALARM_1_ENABLED  0b00000001
    uint8_t c = read_register(DS3231_CONTROL);
    boolean cc = (c & BM_ALARM_1_ENABLED) ;

    if (cc) {
        DPL("Alarm1 is enabled!");
    } else {
        DPL("Alarm1 is disabled!");
    }
    ...
    DPF("Alarm1 s:%i,m:%i h:%i d:%i\n", s, m, h, d);

#define  BM_01_MIN 0b00001111
#define  BM_10_MIN 0b01110000
    uint8_t ss = (s & BM_01_MIN) + 10 * ((s & BM_10_MIN) >> 4);
...
#define BM_10_DATE 0b00110000
    ...
    *alarm = DateTime(yy, mo, dd, hh, mm, ss);

//        DPL("***********************");
    Serial.printf("S:%i M:%i H:%i D:%i\n", s,m, h, d);
    Serial.printf("%i:%i:%i  - DD:%i MO:%i YY:%i \n", hh, mm,ss, dd, mo, yy);
    DPL("***********************");
